...
import { cellAround, CellAttrs, RowAttrs, pointsAtCell } from './util';
  ...
  const plugin = new Plugin<ResizeState>({
    ...
    state: {
      init(_, state) {
        ...
        const rowName = tableNodeTypes(state.schema).row.name;
        if (View && nodeViews) {
          nodeViews[tableName] = (node, view) => {
            return new View(node, cellMinWidth, cellMinHeight, view);
          };
        }
        if (RowView && nodeViews) {
          nodeViews[rowName] = (node, view) => {
            return new RowView(node, cellMinHeight, view);
          };
        }
    ...
    props: {
      ...
      handleDOMEvents: {
        ...
        mousedown: (view, event) => {
          handleMouseDown(view, event, cellMinWidth, defaultCellMinWidth, cellMinHeight, defaultCellMinHeight);
        },
      },
...
function handleMouseDown(
  ...
  function move(event: MouseEvent): void {
    ...
    if (pluginState.dragging && pluginState?.direction === 'horiz') {
      ...
      displayRowHeight(view, pluginState.activeHandle, dragged, defaultCellMinHeight);
    }
  }
